...
static void indicate_timer_timeout(struct k_timer *timer_id)
{
	bt_le_adv_stop();
  k_sleep(K_SECONDS(5)); // Esperar 5 segundo antes de reiniciar el advertising
	k_work_submit(&adv_work);
  LOG_INF("Advertising restarted after timeout");
}

K_TIMER_DEFINE(adv_timeout_timer, indicate_timer_timeout, NULL);
...
static void adv_work_handler(struct k_work *work)
{
  ...
  k_timer_start(&adv_timeout_timer, K_SECONDS(5), K_NO_WAIT);
